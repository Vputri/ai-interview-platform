...
- **Jalur tambahan, ditemuin belakangan pas testing manual end-to-end** (bukan cuma dari baca kode): `api/app/channels/audio_websocket_middleware.rb:387-388` (`handle_gemini_close`) — pas koneksi **Rails↔Gemini** (bukan browser↔Rails) gagal permanen (misal nama model Gemini basi/404), backend ngirim `type: 'session_ended', reason: 'error'` ke browser. Frontend cuma ngecek `type`, gak pernah ngecek `reason` — jadi kegagalan di sisi backend ini KE-SAMARKAN jadi "Interview Complete" juga, walau AI gak pernah sempet ngomong sepatah kata pun. Kelas bug sama persis, titik masuk beda (backend-initiated, bukan frontend-observed) — persis seam "data computation ke frontend presentation" yang diminta brief buat dicek.
