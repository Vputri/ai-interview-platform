...
export default function AssessmentListPage() {
  ...
  const filteredAssessments = useMemo(() => {
    const list = assessments.filter((a) => {
      ...
      if (!matchRole || !matchSkill) return false;

      if (statusFilter === "all") return true;
      if (statusFilter === "active") return a.latest_session?.status === "active";
      if (statusFilter === "awaiting") return a.latest_session?.status === "pending" || !a.latest_session;
      if (statusFilter === "ended") return a.latest_session?.status === "ended";
  ...
  const handleStatusChange = (status: "all" | "active" | "awaiting" | "ended") => {
  ...
  const hasActiveFilters = Boolean(searchRole.trim()) || selectedSkill !== "all" || statusFilter !== "all" || sortBy !== "newest";
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-28 sm:pb-8">
      ...
      <div className="flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-3 bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-2xs">
        ...
        {/* Filter 4: Status Tabs */}
        <div className="flex items-center gap-1 p-1 bg-slate-100 rounded-xl overflow-x-auto self-start xl:self-center shrink-0">
          {[
            { id: "all", label: "All" },
      ...
      {loading ? (
        ...
        <div className="border-2 border-dashed rounded-3xl p-12 text-center space-y-4 bg-white">
          ...
          <Button onClick={() => navigate("/assessments/new")} className="bg-primary text-white shadow-xs">
                  ...
                  <CardHeader className="p-5 pb-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-primary/15 to-primary/5 flex items-center justify-center shrink-0 border border-primary/20 group-hover:scale-105 transition-transform">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 text-xs text-muted-foreground mt-0.5">
                            ...
                            <span>{a.time_limit_min} mins</span>
                            <span>·</span>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-sm text-muted-foreground">
              Are you sure you want to delete <span className="font-semibold text-slate-800">"{assessmentToDelete?.name}"</span>? All associated candidate sessions and interview links will also be removed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
