...
export default function AssessmentInvitePage() {
  ...
  // Mobile Infinite Scroll (Default 10 items)
  ...
  useEffect(() => {
    setCurrentPage(1);
    setMobileLimit(10);
  }, [searchCandidate, dateFilter, statusFilter]);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 text-xs text-muted-foreground font-medium mb-1">
              <button type="button" onClick={() => navigate(-1)} className="hover:text-foreground flex items-center gap-1 text-slate-500 transition-colors cursor-pointer">
                  ...
                  <div className="hidden sm:block divide-y divide-slate-100">
                    {paginatedSessions.map((session, i) => (
                      <SessionRow
                        key={session.id}
                        session={session}
                        index={sessions.length - ((currentPage - 1) * pageSize + i)}
                        assessmentId={id!}
                        onCopy={(sid) => {
                  ...
                  <div className="sm:hidden divide-y divide-slate-100">
                    ...
                    {filteredSessions.length > 0 && mobileLimit >= filteredSessions.length && (
                      <p className="py-3 text-center text-[11px] text-muted-foreground border-t border-slate-100">
                        All {filteredSessions.length} candidates loaded
                      </p>
                    )}
                  </div>
                </>
