...
export default function VacancyEditPage() {
  ...
  const onSubmit = async (data: VacancyFormValues) => {
    ...
    try {
      ...
      setError(e?.response?.data?.errors?.[0]?.message ?? "Failed to save changes.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-28 sm:pb-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground font-medium">
            ...
            <span>/</span>
            <span className="text-slate-900">Edit Vacancy</span>
          </div>
      ...
      {/* ── Form Grid ── */}
      <form id="vacancy-edit-form" onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border-slate-200/80 shadow-xs">
            <CardHeader className="py-4 px-5 border-b border-slate-100 bg-slate-50/50">
              ...
              <CardDescription className="text-xs">
                Position title for Fit/Gap benchmarking
              </CardDescription>
            </CardHeader>
          ...
          <Card className="border-slate-200/80 shadow-xs">
            <CardHeader className="py-4 px-5 border-b border-slate-100 bg-slate-50/50 flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription className="text-xs">
                  Target proficiency levels required for this role
                </CardDescription>
              </div>
            ...
            <CardContent className="p-5 space-y-4">
              {fields.length === 0 ? (
                <div className="rounded-xl border-2 border-dashed border-slate-200 p-8 text-center space-y-3 bg-slate-50/40">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-900">No skill expectations configured</p>
          ...
          <Card className="border-slate-200/80 shadow-xs">
            <CardHeader className="py-4 px-5 border-b border-slate-100 bg-slate-50/50">
              ...
              <CardDescription className="text-xs">
                Context used in AI Fit/Gap narrative generation
              </CardDescription>
            </CardHeader>
            <CardContent className="p-5 space-y-4">
              <div className="space-y-1.5">
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 z-30 bg-background/95 backdrop-blur-sm border-t px-4 py-3 flex gap-2" style={{ bottom: "64px" }}>
        ...
        <Button form="vacancy-edit-form" type="submit" className="flex-1 h-10 bg-primary text-white font-semibold" disabled={!isFormValid || submitting}>
          {submitting && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
          Save Changes
        </Button>
      </div>
